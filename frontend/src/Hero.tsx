  ...
  useEffect(() => {
    ...
    const setup = () => {
      ...
      const tl = gsap.timeline({
        ...
        defaults: { ease: 'none' }, // Changed to 'none' for linear, video-like sequence
      })

      // We space out the animations evenly across the scroll duration
      ...
      tl.to([ch1TitleRef.current, ch1SubRef.current],
        { opacity: 0, y: -40, filter: 'blur(8px)', duration: 0.1 },
        0.25)

      tl.fromTo(ch2TitleRef.current,
        { opacity: 0, letterSpacing: '0.5em', filter: 'blur(18px)' },
        { opacity: 1, letterSpacing: '0.2em', filter: 'blur(0px)', duration: 0.1 },
        0.35)
      ...
      tl.fromTo(ch3TitleRef.current,
        ...
        { opacity: 1, scale: 1,    filter: 'blur(0px)', duration: 0.15 },
        0.75)
